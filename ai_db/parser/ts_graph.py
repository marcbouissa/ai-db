...
def _is_callback_call(node: Node) -> bool:
    ...
    if parent and parent.type in ("call", "argument_list"):
        ...
        if grandparent and grandparent.type == "call":
            ...
            if func_node:
                func_name = func_node.text.decode("utf-8", errors="replace") if hasattr(func_node, 'text') else ""
        ...
        if parent.type in ("function_definition", "function_declaration", "function_item",
            ...
            for child in parent.children:
                if child.type == "decorator":
                    deco_name = child.text.decode("utf-8", errors="replace") if hasattr(child, 'text') else ""
...
def _get_guard(node: Node) -> str | None:
    ...
    while parent:
        if parent.type in ("if_statement", "while_statement", "for_statement"):
            ...
            if condition_node:
                text = condition_node.text.decode("utf-8", errors="replace") if hasattr(condition_node, 'text') else ""
                return text[:50]
            return "condition"
    ...
    if node.type == "call":
        ...
        if func and func.type == "attribute":
            ...
            while attr_node.type == "attribute":
                ...
                if attr:
                    parts.append(attr.text.decode("utf-8", errors="replace") if hasattr(attr, 'text') else "")
                attr_node = attr_node.child_by_field_name("object")
            if attr_node and attr_node.type == "identifier":
                parts.append(attr_node.text.decode("utf-8", errors="replace") if hasattr(attr_node, 'text') else "")
            if parts:
...
def extract_graph(filepath: str, content: str) -> tuple[list[dict], list[dict], list[tuple[int, int, str]]]:
    ...
    for pattern_idx, captures in cursor.matches(tree.root_node):
        # Collect base class names for inherit refs
        base_names = {}
        if "base" in captures:
            for i, node in enumerate(captures.get("base", [])):
                base_names[i] = source[node.start_byte:node.end_byte].decode("utf-8", errors="replace")
                    ...
                    if ref_type == "call":
                        ...
                        while parent:
                            if parent.type in ("function_definition", "function_declaration", "function_item",
                                ...
                                name_node = parent.child_by_field_name("name")
                                if name_node:
                                    caller_name = source[name_node.start_byte:name_node.end_byte].decode("utf-8", errors="replace")
...
# Handle case where call is inside a block that's a method body
                        ...
                        # Get base class name from the corresponding base capture
                        base_name = base_names.get(0, "")
                        caller_name = "module"
                        parent = node.parent
                        while parent:
                            if parent.type in ("class_definition", "class_declaration", "class_specifier",
                                ...
                                break
                            parent = parent.parent
                        # For inherit refs, the base_name is the class name being inherited
                        caller_name = _get_qualified_name(base_name, parent, source, lang, symbols_by_line)
